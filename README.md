...
# Описание проекта
Данный репозиторий содержит web-сервис написанный на с# для взаимодействия между информационными базами 1с. Для корректной работы которого, необходимо установить его на web-сервер(в нашем случае IIS). При выгрузки сервиса необходимо настроить файл My.config(прописать ссылки на ифнормационные базы 1с и ссылку на сервар с Apache kafka). Кроме публикации web-сервиса, информационные базы 1с также публикуются на web-сервере(в нашем случае на Apache web-server) для их корректного общения. Подробная установка и настройка Apache web-server для публикации 1с изложена в разделе [Установка Web-сервера](https://github.com/Alekseim1500/WebServiceFor1c#установка-web-сервера). Для хранения сообщений послучаемых от 1с web-сервисом и отправки от сервиса к 1с используется брокер сообщений(в нашем случае Apache Kafka). Подробная информация про Apache Kafka и её корректная установка и настройка для работы с сервисом c# размещена в разделе [Инструкция к работе с Apache Kafka](https://github.com/Alekseim1500/WebServiceFor1c/blob/main/README.md#инструкция-к-работе-с-apache-kafka). Суть проекта заключается в том, что при добавлении нового элемента в документ или справочник(объекты, с которыми мы неосредственно работали) этот объект отправляется на web-сервис в зависимости от настроек журнала регистраций из My.config. После получаения объекта web-сервисом, объект должен быть отправлен в другую информационную базу 1с в соответствующий документ или справончик. Брокер сообщений Apache Kafka получает на хранение объект после того, как он был передан от 1с к web-сервису. Полученные объекты распределяются по топикам в Apache Kafka, к которым обращается web-сервис для получения данных на отправку в другую инофрмационную базу 1с.  
